...
public class BlackFormula {
  ...
  public static DoubleDerivatives price_Sad(
    ...
    double nPdfpPlus = NORMAL.probability(omega * dPlus);
    for (int loopinput = 0; loopinput < nbInputs; loopinput++) {
      nPlusDot[loopinput] = nPdfpPlus * omega * dPlusDot[loopinput];
    }
    double[] nMinusDot = new double[nbInputs];
    double nPdfdMinus = NORMAL.probability(omega * dMinus);
    for (int loopinput = 0; loopinput < nbInputs; loopinput++) {
  ...
  public static DoubleDerivatives price_Aad(
    ...
    double dMinusBar = NORMAL.probability(omega * dMinus) * omega * nMinusBar;
    double dPlusBar = 1.0d * dMinusBar + NORMAL.probability(omega * dPlus) * omega * nPlusBar;
